...
<html>
    ...
    <?php require 'inc/themes/user/header.php' //This will be the navigation ?>
              ...
              <div class="col-md-12 contentbg" style="padding:10px;">
                <div class="contentcontainer">
                    ...
                    <div style="margin-bottom:10px">
                        <a href="games.php" <?php if(empty($sort)) { ?>class="btn btn-primary"<?php 
                       }else{ ?> <?php 
} ?>class="btn btn-default">Latest</a>
                        <a href="games.php?s=rated" <?php if($sort == "rated") { ?>class="btn btn-primary"<?php 
                       }else{ ?> <?php 
} ?>class="btn btn-default">Top Rated</a>
                        <a href="games.php?s=popular" <?php if($sort == "popular") { ?>class="btn btn-primary"<?php 
                       }else{ ?> <?php 
} ?>class="btn btn-default">Popular</a>
                        <a href="games.php?s=random" <?php if($sort == "random") { ?>class="btn btn-primary"<?php 
                       }else{ ?> <?php 
        ...
        <?php
        if(empty($sort)) {
            $sql = "SELECT * FROM games WHERE status = '1' ORDER BY id DESC";
        }
        if($sort == "latest") {
            $sql = "SELECT * FROM games WHERE status = '1' ORDER BY id DESC";
        }
        if($sort == "rated") {
            $sql = "SELECT * FROM games WHERE status = '1' ORDER BY current_votes DESC";
        }
        if($sort == "popular") {
            $sql = "SELECT * FROM games WHERE status = '1' ORDER BY view_count DESC";
        }
        if($sort == "random") {
            $sql = "SELECT * FROM games WHERE status = '1' ORDER BY rand()";
        }
