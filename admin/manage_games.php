...
<body>
    ...
    <?php if ($page == "delete") { 
        ...
        if($_POST['deleteuser']) {
            activitylog(''.$in['username'].'', 'deleted '.$user1['title'].'', ''.time().'', 'Admin');
            $sql = "DELETE FROM `games` WHERE id = '".$user1["id"]."'";
            $dbh->exec($sql);
        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="panel panel-default">
                    <div class="panel-heading">Delete <?php echo $user1['title']; ?></div>
                    <div class="panel-body">
                    <form method="post">
                    <div class="form-group">
                <label for="viewprofile">Are you sure you would like to delete this game? This can not be undone!!<br />
                    
                </label>
              </div>
              <input type="submit" style="float:left;margin-right:10px;"class="btn btn-primary" value="Delete game" name="deleteuser"> 
    ...
    <?php if ($page == "add") {
        ...
        ?>
                      ...
                      <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="name" name="title" placeholder="Game name" value="<?php echo $user1['title']; ?>">
                      </div>
        ...
        foreach ($nodes2 as $n1) {
            ...
            ?>
                        <option value="<?php echo $n1['id']; ?>" <?php if($user1['category_id'] == "".$n1['id']."") { echo 'selected'; 
                       } ?>><?php echo $n1['title']; ?></option>
        <?php } ?>
                    </select>
                </label>
                </div>
                ...
                <label for="status">Status<br />
                    <select class="form-control" id="status" name="status">
                        ...
                        <option value="1" <?php if($user1['status'] == "1") { echo 'selected'; 
                       } ?>>Active</option>
                        <option value="0" <?php if($user1['status'] == "0") { echo 'selected'; 
                       } ?>>Inactive</option>
                    </select>
                </label>
              </div>
              
              <input type="submit" style="float:left;"class="btn btn-primary" value="Add Game" name="updategame_settings">
              </form>
            </div></div></div>
    ...
    <?php if ($page == "edit") {
        ...
        ?>
        <div class="panel panel-default">
                <div class="panel-heading">Edit: <?php echo $user1['title']; ?></div>
        </div>
        <div class="row"><form method="post">
            <div class="col-lg-6">
                <div class="panel panel-default">
        ...
        if(isset($_POST['updategame_settings'])) {
            if($in["username"]) {
                ...
                $currenturl = 'http' . (isset($_SERVER['HTTPS']) ? 's' : '') . '://' . "{$_SERVER['HTTP_HOST']}";
                activitylog(''.$in['username'].'', 'edited '.$user1['title'].'', ''.time().'', 'Admin');
                            
                $sql = $dbh->prepare("UPDATE games SET title='".$_POST['title']."', description='".$_POST['description']."', category_id='".$_POST['category_id']."', status='".$_POST['status']."' WHERE id=".$user1['id']."");
                $sql->execute();
                            
                $success = "".$user1['title']." has been updated!";
        ...
        $new_name = $location.time()."-".rand(1000, 9999)."-".$_FILES["imageToUpload"]["name"];
        ...
        if(isset($_POST["uploadimage"])) {
            ...
            if ($uploadOk == 0) {
                ...
                if (move_uploaded_file($_FILES["imageToUpload"]["tmp_name"], $target_file2)) {
                    ...
                    $sql = $dbh->prepare("UPDATE games SET image='".$currenturl."/files/uploads/".$new_name."' WHERE id=".$user1['id']."");
                    $sql->execute();
        ...
        if(isset($_POST['gameimage'])) {
            $sql = $dbh->prepare("UPDATE games SET image='".$_POST['gameimage_text']."' WHERE id=".$user1['id']."");
            $sql->execute();
            $success = "".$currenturl."/".$new_name."";
            header("location: ".$currenturl."".$_SERVER[REQUEST_URI]."&success=game");
        }
...
<div class="cover2 box1">
    <form method="post" enctype="multipart/form-data">
        <input type="text" class="btn btn-default" style="width:80%;text-align:left;float:left;" value="<?php echo $user1['image']; ?>" name="gameimage_text" placeholder="URL to SWF file">
        <input type="submit" style="float:left;"class="btn btn-primary" value="Update "  name="gameimage"><br>
                      ...
                      <div class="form-group">
                        <br><br><label for="name">Game File</label><br>
                        <script type="text/javascript" src="http://code.jquery.com/jquery.min.js"></script>
        ...
        if(isset($_POST["uploaduseravatar"])) {
            ...
            if ($uploadOk == 0) {
                ...
                if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file2)) {
                    ...
                    $sql = $dbh->prepare("UPDATE games SET file='".$currenturl."/files/uploads/".$new_name."', type='Flash' WHERE id=".$user1['id']."");
                    $sql->execute();
                    $success = "".$currenturl."/".$new_name."";
                    header("location: ".$currenturl."".$_SERVER[REQUEST_URI]."&success=fileuploaded");
                } else {
        ...
        if(isset($_POST['gamefileurl'])) {
            $sql = $dbh->prepare("UPDATE games SET file='".$_POST['gamefileurl_text']."', type='Flash' WHERE id=".$user1['id']."");
            $sql->execute();
            $success = "".$currenturl."/".$new_name."";
            header("location: ".$currenturl."".$_SERVER[REQUEST_URI]."&success=game");
        }
        ...
        if(isset($_POST['HTML5-url'])) {
            $sql = $dbh->prepare("UPDATE games SET file='".$_POST['html5url-text']."', type='HTML5-url' WHERE id=".$user1['id']."");
            $sql->execute();
            $success = "".$currenturl."/".$new_name."";
            header("location: ".$currenturl."".$_SERVER[REQUEST_URI]."&success=game");
        }
        if(isset($_POST['HTML5embed'])) {
            $sql = $dbh->prepare("UPDATE games SET file='".$_POST['HTML5embed-textarea']."', type='HTML5' WHERE id=".$user1['id']."");
            $sql->execute();
            $success = "".$currenturl."/".$new_name."";
            header("location: ".$currenturl."".$_SERVER[REQUEST_URI]."&success=game");
        }
...
<form method="post" enctype="multipart/form-data">
    ...
    <input type="submit" style="float:left;"class="btn btn-primary" value="Upload SWF" name="uploaduseravatar"><br>
</form>
</div>
</div>
...
<div class="2 box">
    <form method="post" enctype="multipart/form-data">
        <input type="text" class="btn btn-default" style="width:80%;text-align:left;float:left;" value='<?php echo $user1['file']; ?>' name="gamefileurl_text" placeholder="URL to SWF file">
...
<div class="3 box">
    <form method="post" enctype="multipart/form-data">
        <input type="text" class="btn btn-default" style="width:80%;text-align:left;float:left;" value='<?php echo $user1['file']; ?>' name="html5url-text" placeholder="URL to html5 file">
...
<div class="4 box">
    <form method="post" enctype="multipart/form-data">
        <textarea type="text" class="btn btn-default" style="width:100%;text-align:left;height:200px;" value="<?php echo $user1['file']; ?>" name="HTML5embed-textarea" placeholder="Enter game scripts here"></textarea>
